...
class PitmanYorProcess(BaseWeights):
    def __init__(self, pyd=0, theta=1, rng=None):
        super().__init__(rng=rng)
        assert -pyd < theta, "theta param must be greater than -pyd"
        self.pyd = pyd
        self.theta = theta
        self.v = np.array([], dtype=np.float64)

    def structure_log_likelihood(self, v=None, pyd=None, theta=None):
        if v is None:
            v = self.v
        if pyd is None:
            pyd = self.pyd
        if theta is None:
            theta = self.theta
        return self.weight_log_likelihood(v=v, pyd=pyd, theta=theta)

    def weight_log_likelihood(self, v=None, pyd=None, theta=None):
        if v is None:
            v = self.v
        if pyd is None:
            pyd = self.pyd
    ...
    def random(self, size=None):
        ...
        if len(self.d) == 0:
            ...
            self.v = self.rng.beta(a=1 - self.pyd,
                                   b=self.theta + pitman_yor_bias * self.pyd,
                                   size=size)
            ...
            self.v = self.rng.beta(
                a=1 - self.pyd + a_c,
                b=self.theta + pitman_yor_bias * self.pyd + b_c
            )
            self.w = self.v * np.cumprod(np.concatenate(([1],
                                                         1 - self.v[:-1])))
            if size is not None:
    ...
    def tail(self, x):
        ...
        while w_sum < x:
            v_to_append = self.rng.beta(
                a=1 - self.pyd,
                b=self.theta + self.get_size() * self.pyd,
                size=1)
